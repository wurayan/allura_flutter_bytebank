...
// implementar uma verificação que impeça a repetição de ids
//desnecessa´rio porque não há implementação nem tratamento de ids no resto do código inteiro
...
class ContactDao {
  ...
  Future<List> find(int id) async {
    ...
    final List<Map<String, dynamic>> result = await db.query(_tableName, where: '$_id = ?', whereArgs: [id]);
  ...
  Future<dynamic> save(Contacts contact) async {
    ...
    var idExists = await find(contact.id);
    if (idExists.isEmpty) {
